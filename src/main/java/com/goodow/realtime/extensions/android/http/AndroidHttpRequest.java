...
final class AndroidHttpRequest implements HttpRequest {
	private class RequestTask
			extends
				AsyncTask<Void, Void, AndroidHttpResponse> {
	...
	private static final Logger LOG = Logger
			.getLogger(com.google.api.client.http.HttpTransport.class.getName());
	...
	public void executeAsync(HttpRequestCallback callback, String content) {
		...
		try {
			httpRequest = requestFactory.buildRequest(method, new GenericUrl(
					HttpTransport.CHANNEL + url), content == null
					? null
					: new ByteArrayContent("application/json; charset=utf-8",
							content.getBytes("UTF-8")));
		} catch (IOException e) {
			callback.onFailure(e);
		}
		// 多线程
		new RequestTask(callback)
